...
Cybercrime investigation platform: forensic analysis tools, expert directory, episode/case audio player with transcripts, analytics dashboard. Next.js 15 (App Router) + React 19 + TypeScript + TailwindCSS, deployed on Vercel from `main`.
...
- **Package manager**: pnpm (`pnpm-lock.yaml` is the source of truth — don't introduce `package-lock.json` changes as the primary lockfile)
...
There is no test script yet and no `.github/workflows` CI — say so explicitly if a task implies test coverage or CI enforcement that doesn't exist.
