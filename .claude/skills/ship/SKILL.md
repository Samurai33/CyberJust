...
Release checklist for this repo: Next.js 16 app, deployed to Vercel from `main`. CI (`.github/workflows/ci.yml`) runs `pnpm type-check` + `pnpm build` on every PR and push to `main` — this skill mirrors those checks locally before you push, plus a few things CI doesn't cover. No test script exists in `package.json`.
...
4. Wait for CI (`type-check` + `build`) to go green on the PR before merging
5. After merge, Vercel auto-deploys from `main` — confirm the deploy succeeds at the dashboard/`vercel.app` URL, don't assume

## What This Skill Does Not Do

- There is no test suite yet — this skill cannot verify behavior beyond type-check/build. If you're shipping a risky change, say so explicitly rather than implying test coverage exists.
- CI does not lint — ESLint isn't configured in this repo (see the tracked follow-up issue). `pnpm type-check` catches type errors but not style/correctness lint rules.
